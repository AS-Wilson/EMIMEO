...
lambdamin = 1.2e-6;                                %%% Minimum wavelenght
lambdamax = 1.8e-6;                                %%% Maximum wavelenght
...
beta1 = vg.^-1;
beta2 = diff(beta1)./diff(omega(2:end));

%% Plots
%%%%%% Beta'' with respect to lambda %%%%%%
figure('Renderer', 'painters', 'Position', [400 50 900 700])
hold on
plot(lambda(1:end-2),beta2,'-g','LineWidth',2.5)
[BETA2_EQ0,IN] = min(beta2.^2);
...
xlabel('Wavelenght(m)');
ylabel('Beta2(s^2/m)');
axis([lambdamin lambdamax -4e-26 4e-26])
ax = gca;
ax.XAxis.Exponent = -6;
ax.YAxis.Exponent = -26;
