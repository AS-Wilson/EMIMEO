...
envelope = (intensity * np.exp(-(((time - time_shift) ** 2) / (2 * (init_waist ** 2))))) + \
           (intensity * np.exp(-(((time + time_shift) ** 2) / (2 * (init_waist ** 2)))))
...
# Save graph in this location, bbox_inches='tight' removes any spare whitespace from the plot padding
plt.savefig('../Graphs/split-step-gvd-2d-time.pgf', format='pgf', bbox_inches='tight', pad_inches=0)
...
plt.savefig('../Graphs/split-step-gvd-2d-freq.pgf', format='pgf', bbox_inches='tight', pad_inches=0)


## 3D TIME DOMAIN GRAPH

index_0 = round((-40-time[0]) / sample_period)
index_1 = round((40-time[0]) / sample_period)


# Create a mesh matrix to plot signal values against
T, Z = np.meshgrid(time[index_0:index_1], z_axis)
...
ax.plot_surface(T, Z, abs(pulse_output_time[:, index_0:index_1]), rstride=1, cstride=1, cmap='viridis', edgecolor='none')
...
ax.auto_scale_xyz([-41, 41], [-5, 5], [0, 1.1])
ax.set_box_aspect((1.5, 2.0, 0.6))

plt.tight_layout(pad=0.8) # Place everything slightly closer together

# Save graph in this location
plt.savefig('../Graphs/split-step-gvd-3d-time.pgf', format='pgf', bbox_inches='tight', pad_inches=0)



## 3D FREQUENCY DOMAIN GRAPH

index_0 = round((-3-ang_freq_axis[0]) / ang_freq_interval)
index_1 = round((3-ang_freq_axis[0]) / ang_freq_interval)

# Create a mesh matrix to plot signal values against
S, Zs = np.meshgrid(ang_freq_axis[index_0:index_1], z_axis)
...
# Plot the signal as it propagates
ax.plot_surface(S, Zs, abs(pulse_output_spectrum[:, index_0:index_1])**2, rstride=1, cstride=1, cmap='viridis', edgecolor='none')

# Colouring the background of the graph
fig.patch.set_facecolor('white')
ax.axes.set_facecolor('white')
ax.axes.w_xaxis.set_pane_color((0.95, 0.95, 0.95, 0.95))
ax.axes.w_yaxis.set_pane_color((0.95, 0.95, 0.95, 0.95))
ax.axes.w_zaxis.set_pane_color((0.95, 0.95, 0.95, 0.95))

# Title and axis labels
ax.axes.set_title(r'3D Plot of Signal Propagation in Freq. and Space')
ax.axes.set_xlabel(r'Angular Frequency, $\omega$')
ax.axes.set_ylabel(r'Propagation distance, z')
ax.axes.set_zlabel(r'$\tilde{F}(\omega)$')

# Aspect ratio settings
# ax.auto_scale_xyz([-5, 5], [-5, 5], [0, 4.1])
ax.set_box_aspect((1.5, 2.0, 0.5))

plt.tight_layout(pad=0.8) # Place everything slightly closer together

# Save graph in this location
plt.savefig('../Graphs/split-step-gvd-3d-freq.pgf', format='pgf', bbox_inches='tight', pad_inches=0)
