...
%				\draw[help lines] (0,0) grid (11,11);
	...
	\subsection{A Quick Example}
		...
		\begin{equation}
			\label{eqn:quantum_flux} \mathlarger{\text{\normal Flux}, F = \frac{\text{\normal no. of quanta}, n}{\text{\normal area}, a \times \text{\normal time}, t}}
		\end{equation}
		
		\vspace{5mm}
		
		Given the definition of intensity, shown in \ref{eqn:intenisty_def}, let's say the intensity and wavelength of the light beam are $I=2w/m^2$ and $\lambda=250nm$ respectively. We'll begin by calculating the energy of one single quanta (Equation \ref{eqn:ex_1_energy_single_quant}), then we'll calculate the number of quanta emitted by our light beam (Equation \ref{eqn:ex_1_quant_in_beam}), and finally combine those two equations to get the total energy in the beam (Equation \ref{eqn:ex_1_total_energy}). \linebreak
	
		\begin{align}
			& \text{Energy of a single quanta} \nonumber\\
			& \mathlarger{ E = \Planckconst \nu = \frac{\Planckconst c}{\lambda} = \frac{12.4 \times 10^3 eV \cdot \r{A}}{250 \times 10^{-9} m } \text{ \tiny Remember an Angstrom is $1 \times 10^{-10}$ metres}} \nonumber\\
			& \label{eqn:ex_1_energy_single_quant} \mathlarger{ \implies \text{1 Quanta } = 4.96 eV} \\
			& \text{Number of quanta in the beam} \nonumber\\
			& \label{eqn:intenisty_def} \mathlarger{ \text{\normal Intensity }= 2 \frac{w}{m^2} = 2 \frac{J}{s \times m^2} } \\
			& \mathlarger{ \therefore \text{\normal No. of Quanta /$m^2s$} = 2 \frac{J}{s \times m^2} \times \frac{1}{\Planckconst \nu} }
		\end{align}
