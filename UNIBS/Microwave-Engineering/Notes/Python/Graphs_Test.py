...
def gaussian_demo():
    ...
    ## REVERSING SPECTRUM
    inv_gaussian_pulse = ifft(ifftshift(gaussian_pulse_spectrum))*ang_freq_interval/np.sqrt(2.0*np.pi)*no_of_samples
    ...
    ## INVERSE FOURIER PULSED GAUSSIAN PLOT
    plt.figure()
    
    plt.plot(time, inv_gaussian_pulse, color='green')
    ...
    # Titles and labels
    plt.title(r'Pulsed Gaussian Signal from IFFT')
    plt.xlabel(r'Time (Seconds)')
    plt.ylabel(r'Intensity (Unitless)')
