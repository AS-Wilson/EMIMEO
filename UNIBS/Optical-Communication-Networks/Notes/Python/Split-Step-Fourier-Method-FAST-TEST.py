...
def gaussian(time, FWHM, t_0, intensity=1, t_shift=0):
    ...
    try:
        ...
        signal = np.matrix(intensity * np.exp(- ((time - t_shift) ** 2 / 2 * t_0 ** 2)))
        return signal
...
def split_step_fourier_method(input_spectrum, ang_freq, ang_freq_interval, z_step_size, beta_2=0, gamma=0):
    ...
    output_spectrum = np.r_[complex(input_spectrum, 0)]
    output_time = np.r_[ifft(ifftshift(input_spectrum)) * ang_freq_interval / np.sqrt(2.0 * np.pi) * len(input_spectrum)]
    ...
    while range(len(input_spectrum)-1, 0, -1):

        for i in range(no_of_samples):
            # Dispersive (Linear) Step
            output_spectrum = np.row_stack(output_spectrum, np.concatenate(  )
