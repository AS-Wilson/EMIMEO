...
IN_11 = intensity * np.exp(-(((time + 10) ** 2) / (2 * (init_waist ** 2))))
IN_12 = 0 * np.exp(-(((time - 10) ** 2) / (2 * (init_waist ** 2))))
IN_13 = intensity * np.exp(-((time ** 2) / (2 * (init_waist ** 2))))

envelope_1 = IN_11 + IN_12 + IN_13


IN_21 = intensity * np.exp(-(((time - 10) ** 2) / (2 * (init_waist ** 2))))
IN_22 = intensity * np.exp(-(((time + 10) ** 2) / (2 * (init_waist ** 2))))
IN_23 = intensity * np.exp(-((time ** 2) / (2 * (init_waist ** 2))))

envelope_2 = IN_21 + IN_22 + IN_23

envelope_unshift = envelope_1 + envelope_2
envelope = [envelope_unshift[0] * (1 * np.exp(complex(0, -10 * (time[0]))))]

for i in range(1, no_of_samples):
    envelope.append(envelope_unshift[i] * (1 * np.exp(complex(0, (-10 * time[i])))))

envelope_unshift = np.array(envelope_unshift)
envelope = np.array(envelope)

envelope_1 = np.array(envelope_1)
envelope_2 = np.array(envelope_2)
...
plt.plot(time, abs(envelope_1), color='blue', label='Signal One')
...
# Plot the input envelope vs time
plt.plot(time, abs(envelope_2), color='blue', label='Signal Two')
...
# Plot the input envelope vs time
plt.plot(time, abs(envelope_unshift), color='blue', label='Initial Signal')
...
# Plot the input envelope vs time
plt.plot(time, abs(envelope), color='blue', label='Initial Signal')
