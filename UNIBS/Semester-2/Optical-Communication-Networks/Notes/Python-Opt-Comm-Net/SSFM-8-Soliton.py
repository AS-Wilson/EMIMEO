...
input_signal = 1 / np.cosh(time)


Fundamental_Bright_Soliton = Split_Step_Fibre_Propagation(time, signal_propagation_distance,
                                                          input_signal, no_of_z_samples=200,
                                                          beta_2=-1, gamma=1)
...
plt.plot(Fundamental_Bright_Soliton.time, input_signal, color='blue', label='Initial Signal')

# Plot the output envelope vs time
plt.plot(Fundamental_Bright_Soliton.time, abs(Fundamental_Bright_Soliton.signal_propagation_time[-1, :]), color='red', label='Final Signal')

# Titles and labels
plt.title(r'Input and Output Envelope, $1^{st}$ Order Bright Soliton - Time Domain')
plt.ylabel(r'$F(t)$')
plt.xlabel(r'Time, t')
...
plt.plot(Fundamental_Bright_Soliton.ang_freq_axis, Fundamental_Bright_Soliton.abs_input_spec, color='green', label='Initial Spectrum')

# Plot the output envelope vs time
plt.plot(Fundamental_Bright_Soliton.ang_freq_axis, abs(Fundamental_Bright_Soliton.signal_propagation_spec[-1, :]), color='orange', label='Final Spectrum')

# Titles and labels
plt.title(r'Input and Output Envelope, $1^{st}$ Order Bright Soliton - Freq. Domain')
...
start_index = round((-10 - Fundamental_Bright_Soliton.time[0]) / sample_period)
end_index = round((10 - Fundamental_Bright_Soliton.time[0]) / sample_period)

## 3D TIME DOMAIN GRAPH
make_3d_graph(Fundamental_Bright_Soliton.time[start_index:end_index],
              abs(Fundamental_Bright_Soliton.signal_propagation_time[:, start_index:end_index]),
              Fundamental_Bright_Soliton.z_axis, title=r'$1^{st}$ Order Bright Soliton Prop. - Time Domain', x_lab=r'Time, t',
...
start_index = round((-4 - Fundamental_Bright_Soliton.ang_freq_axis[0]) / Fundamental_Bright_Soliton.ang_freq_interval)
end_index = round((4 - Fundamental_Bright_Soliton.ang_freq_axis[0]) / Fundamental_Bright_Soliton.ang_freq_interval)

## 3D FREQUENCY DOMAIN GRAPH
make_3d_graph(Fundamental_Bright_Soliton.ang_freq_axis[start_index:end_index],
              abs(Fundamental_Bright_Soliton.signal_propagation_spec[:, start_index:end_index]), Fundamental_Bright_Soliton.z_axis,
              title=r' $1^{st}$ Order Bright Soliton Prop. - Freq. Domain', x_lab=r'Angular Frequency, $\omega$',
              y_lab=r'Propagation distance, z', z_lab=r'$\tilde{F}(\omega)$',
              auto_scale=([-4, 4], [0, 10], [0, 3.5]),
